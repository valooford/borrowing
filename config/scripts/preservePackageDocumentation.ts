...
/** */
async function preservePackageDocumentation(srcPath: string, distPath?: string) {
  ...
  if (parserContext.docComment.modifierTagSet.hasTagName('@packageDocumentation')) {
    const dtsPath = path.join('dist', distPath ?? `${srcPath.slice(0, -path.extname(srcPath).length)}.d.ts`)
    ...
    await fs.writeFile(dtsPath, `${parserContext.docComment.emitAsTsdoc()}\n${dts.toString()}`, 'utf-8')
  }
}
